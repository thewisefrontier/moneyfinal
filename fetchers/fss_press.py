...
def main():
    ...
    try:
        res = fss_open_api_get('bodoInfo', AUTH_KEY, days_back=14)
        res.raise_for_status()
        data = res.json()
        items = data.get('result', {}).get('list', []) if isinstance(data, dict) else []
        if not items:
            logger.warning("보도자료: 데이터 없음")
            return
        results = []
        for item in items:
            results.append({
        ...
        if results:
            ...
            logger.info(f"✅ 보도자료 {len(results)}건 저장")
    except Exception as e:
        logger.error(f"보도자료 수집 오류: {type(e).__name__} - {e}")
    logger.info("=== 보도자료 수집 완료 ===")
