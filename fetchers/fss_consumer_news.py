...
def main():
    logger.info("=== 금융소보자 뉴스 수집 시작 ===")
    try:
        res = fss_open_api_get('fcnInfo', AUTH_KEY, days_back=30)
        res.raise_for_status()
        data = res.json()
        # 주의: FSS 응답 키는 "response"가 아니라 오타난 "reponse"
        items = data.get('reponse', {}).get('result', []) if isinstance(data, dict) else []
        if not items:
            logger.warning("금융소보자뉴스: 데이터 없음")
            return
        results = []
        for item in items:
            results.append({
        ...
        if results:
            ...
            logger.info(f"✅ 금융소보자뉴스 {len(results)}건 저장")
    except Exception as e:
        logger.error(f"금융소보자뉴스 수집 오류: {type(e).__name__} - {e}")
    logger.info("=== 금융소보자 뉴스 수집 완료 ===")
