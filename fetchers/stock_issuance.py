    ...
    try:
        ...
        for item in items:
            ...
            results.append({
                ...
                'issuance_date': f"{base_dt[:4]}-{base_dt[4:6]}-{base_dt[6:8]}" if len(base_dt)==8 else today_kst(),
                'issuance_type': item.get('issuKindNm',''),
                'issue_price':   float(item.get('issuPrc',0) or 0),
                'issue_amount':  int(item.get('issuAmt',0) or 0),
                'fetched_at':    now_kst()
            })
        return results
