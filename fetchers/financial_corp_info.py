...
def fetch_financial_corps() -> list:
    ...
    try:
        ...
        return [{'stock_code':f"FIN_{item.get('fncoRegNo','')}", 'corp_name':item.get('fncoNm',''), 'industry_name':item.get('fncoClssNm',''), 'address':item.get('fncoAdr',''), 'homepage':item.get('fncoHmpgUrl',''), 'phone':item.get('fncoTlno',''), 'market_type':'금융회사', 'fetched_at':now_kst()} for item in items if item.get('fncoRegNo','')]
...
def fetch_financial_credit() -> list:
    ...
    try:
        ...
        for item in items:
            ...
            results.append({'indicator_code':f"BIS_{item.get('fncoRegNo','')[:10]}", 'indicator_name':f"{item.get('fncoNm','')} BIS비율", 'category':'금융회사건전성', 'value':bis, 'unit':'%', 'signal':'green' if bis>=12 else 'yellow' if bis>=10 else 'red', 'source':'금융위원회 (공공데이터포털)', 'reference_date':today_kst(), 'fetched_at':now_kst()})
        return results
    except Exception as e:
...
def main():
    ...
    if corps: supabase_upsert('corp_info', corps); logger.info(f"✅ 금융회사기본정보 {len(corps)}건")
    credit = fetch_financial_credit()
    if credit: supabase_upsert('market_indicators', credit); logger.info(f"✅ 금융회사재무신용 {len(credit)}건")
