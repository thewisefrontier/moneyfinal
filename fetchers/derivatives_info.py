...
def main():
    ...
    params = {'resultType':'json','numOfRows':20,'pageNo':1,'basDt':base_date}
    try:
        ...
        for item in items:
            ...
            results.append({'indicator_code':f"DERIV_{idx_nm[:15]}", 'indicator_name':idx_nm, 'category':'파생상품', 'value':clpr, 'unit':'pt', 'signal':'red' if flt_rt<=-3 else 'yellow' if flt_rt<=-1 else 'green', 'source':'금융위원회 (공공데이터포털)', 'reference_date':today_kst(), 'summary_text':f"{idx_nm} {clpr:.2f}pt ({flt_rt:+.2f}%)", 'fetched_at':now_kst()})
