...
def main():
    logger.info("=== 금융꿀팁 200선 수집 시작 ===")
    try:
        res = fss_open_api_get('finTip', AUTH_KEY, days_back=30)
        res.raise_for_status()
        data = res.json()
        items = data.get('result', {}).get('list', []) if isinstance(data, dict) else []
        if not items:
            logger.warning("금융꿀팁: 데이터 없음")
            return
        results = []
        for item in items:
            results.append({
        ...
        if results:
            ...
            logger.info(f"✅ 금융꿀팁 {len(results)}건 저장")
    except Exception as e:
        logger.error(f"금융꿀팁 수집 오류: {type(e).__name__} - {e}")
    logger.info("=== 금융꿀팁 200선 수집 완료 ===")
