...
def main():
    ...
    try:
        ...
        results = [{'indicator_code':f"BANK_{item.get('sttsItemNm','')[:20]}", 'indicator_name':item.get('sttsItemNm',''), 'category':'은행통계', 'value':float(item.get('sttsVal',0) or 0), 'unit':item.get('sttsValUnit',''), 'signal':'green', 'source':'금융위원회 (공공데이터포털)', 'reference_date':today_kst(), 'fetched_at':now_kst()} for item in items]
